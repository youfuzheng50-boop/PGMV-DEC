...
from models.wsi_dataset import Wsi_Region
...
def drawHeatmap(model_type,scores, coords, slide_path=None, wsi_object=None, vis_level = -1, **kwargs):
    ...
    if model_type == '聚类结果':
        heatmap = wsi_object.visHeatmap(scores, coords=coords, vis_level=vis_level, **kwargs)
    else: heatmap = wsi_object.Classmap(scores, coords=coords, vis_level=vis_level, **kwargs)
    return heatmap
