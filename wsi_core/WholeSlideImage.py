...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def Classmap(self, categories, coords, vis_level=-1,
                   top_left=None, bot_right=None,
                   patch_size=(224, 224),
                   blank_canvas=False, canvas_color=hex_to_rgb('#F9F9F9'), alpha=0.4,
        ...
        if c is None:
            c = ['#F2AEE6', '#00B050', '#EF8600', '#0097A7', '#7030A0']
        ...
        # --- MODIFICATION 1: 验证输入分数的形状 ---
        # 不再需要 .flatten()
        # categories = categories.flatten()
        if len(categories.shape) != 2:
            raise ValueError(
                f"Input 'categories' must be an (N, num_classes) array of scores, but got shape {categories.shape}")

        # 确保数据类型是浮点型
        if not np.issubdtype(categories.dtype, np.floating):
            print(f"Warning: 'categories' (scores) are not float. Will attempt to cast. Got {categories.dtype}")
            categories = categories.astype(float)

        # 警告：如果分数不在 [0, 1] 范围内（例如 logits），混合结果可能不正确
        if np.min(categories) < 0.0 or np.max(categories) > 1.0:
            print("Warning: Scores appear to be outside the [0, 1] range (e.g., logits).")
            print("         Heatmap intensity may not render correctly. Please apply Softmax first.")
        # --- END MODIFICATION 1 ---

        # 处理坐标范围和区域尺寸
        if top_left is not None and bot_right is not None:
            # 假设 screen_coords 仍然适用
            coords = screen_coords(categories, coords, top_left, bot_right)
        ...
        # 生成热图
        overlay = np.full((h, w, 3), canvas_color, dtype=np.uint8)

        # --- MODIFICATION 2: 获取用于混合的背景色 ---
        # 将画布颜色转换为 numpy 数组以便进行数学运算
        canvas_color_rgb = np.array(canvas_color, dtype=float)
        # --- END MODIFICATION 2 ---

        # 使用一张原始图像或空白画布
        if not blank_canvas:
            img = np.array(self.wsi.read_region(top_left, vis_level, (w, h)).convert("RGB"))
        else:
            img = np.array(Image.new(size=(w, h), mode="RGB", color=canvas_color))

        if segment:
            # --- MODIFICATION 3: 修正 get_seg_mask 的参数 ---
            # 原始代码的 region_size 可能不正确，如果 top_left/bot_right 被指定
            # 我们使用 (w, h) 作为读取区域的大小
            tissue_mask = self.get_seg_mask((w, h), scale, use_holes=use_holes, offset=tuple(top_left))
            # --- END MODIFICATION 3 ---

        # --- MODIFICATION 4: 循环逻辑 ---
        for idx in range(len(coords)):
            # score_vector 是 (num_classes,) 形状的数组
            score_vector = categories[idx]
            coord = coords[idx]

            # 4a. 获取预测类别和置信度
            category = np.argmax(score_vector)
            confidence = score_vector[category]

            # 裁剪以确保 confidence 在 [0, 1] 范围内
            confidence = np.clip(confidence, 0.0, 1.0)

            if 0 <= category < len(c):
                # 4b. 获取该类别的 "全强度" 目标颜色
                target_color = np.array(mcolors.to_rgb(c[category])) * 255

                # 4c. 根据置信度混合目标颜色和画布背景色
                # final_color = (confidence * target_color) + ((1 - confidence) * canvas_color_rgb)
                final_color = (confidence * target_color + (1 - confidence) * canvas_color_rgb).astype(np.uint8)

                x_start, x_end = coord[0], coord[0] + patch_size[0]
                y_start, y_end = coord[1], coord[1] + patch_size[1]

                # 边界检查 (修正：使用 <= 而不是 <，以包含到达边缘的切片)
                if x_start >= 0 and y_start >= 0 and x_end <= w and y_end <= h:
                    # 仅在 segment 为 True 时应用组织掩码
                    if segment:
                        # 确保掩码和切片区域的形状匹配
                        mask_slice = tissue_mask[y_start:y_end, x_start:x_end]
                        overlay_slice = overlay[y_start:y_end, x_start:x_end]

                        # 仅在掩码为True的地方应用颜色
                        overlay_slice[mask_slice] = final_color
                        overlay[y_start:y_end, x_start:x_end] = overlay_slice
                    else:
                        overlay[y_start:y_end, x_start:x_end] = final_color
        # --- END MODIFICATION 4 ---
